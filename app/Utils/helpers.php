...
//        return (($price-$discount) / 100) * $tax; //after discount decrease
    ...
    //     // Check if user is super admin (optional: move this check to top if you prefer)
    ...
    //     // Make sure role and permissions are present
    //     if (!$user_role || $user_role->status != 1 || !isset($user_role->module_access)) {
    //         return false;
    //     }
    
    //     $permission = json_decode($user_role->module_access, true);
    
    //     return in_array($mod_name, (array) $permission);
    // }
    public static function module_permission_check($mod_name, $guard = 'admin')
{
    ...
    // Check for seller
    elseif ($guard === 'seller') {
        // You can customize seller role access logic here
        $seller_role = $user->role;

        if (!$seller_role || $seller_role->status != 1 || !isset($seller_role->module_access)) {
            return false;
        }

        $permission = json_decode($seller_role->module_access, true);
    }
